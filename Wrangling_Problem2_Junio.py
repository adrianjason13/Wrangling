...
messy_data = Box1.append(Box2).reset_index().drop(columns = ['index'])
tidy_1 = messy_data.pivot_table(index = 'Box',columns = 'Dimension',values = 'Value').reset_index()
data3 = {'Box':['Box1','Box2'],'Volume':[6*4*2,5*3*4]}
Volume = pd.DataFrame(data3,columns = ['Box','Volume'])
tidy = pd.merge(tidy_1,Volume, how='right',on='Box')
print(tidy)